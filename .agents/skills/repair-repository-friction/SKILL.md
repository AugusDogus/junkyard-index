---
name: repair-repository-friction
description: Repair a reproducible defect in Junkyard Index code discovered while running a supported Junkyard Index workflow. Use after a minimized case reproduces on the current default branch. Do not use when the only failure is invalid input with an actionable diagnostic, documented unsupported behavior, an unresolved product decision, or a fix owned by another repository.
---

# Repair Junkyard Index repository friction
...
Receive or reconstruct this evidence before editing repository code:

- the parent task and how the defect blocks or weakens it;
- the smallest test or command that reproduces the failure;
...
Classify the failure first. Return it to the discovering agent when the input is
invalid and Junkyard Index already names the problem and recovery, the behavior
is documented as unsupported, or the failure comes only from local setup outside
the repository's control. An opaque or internal error caused by invalid input is
a diagnostic defect; repair the diagnostic without accepting the input. Ask for
a decision when the contract is ambiguous or a fix would choose new product
behavior. Ask before changing a submodule, another repository, or an external
upstream.
...
3. Trace the failure to its root cause and fix Junkyard Index code. Preserve the
   supported contract. If the code rejected bad input but gave an opaque error,
   make the diagnostic name the failed operation, bad value or state, expected
...
5. Inspect the complete diff, commit only the repair and its evidence, push the
   dedicated branch, and open or update one Junkyard Index pull request. The
   pull request must carry the reproduction, root cause, fix, regression
   receipt, affected validation, and impact on the parent task.
...
Only that retest closes the repository-friction branch of the parent task.
