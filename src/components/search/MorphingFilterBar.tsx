...
import { ArrowUpDown, Calendar, Filter, MapPin } from "lucide-react";
...
export const MorphingFilterBar = forwardRef<
  HTMLDivElement,
...
>(function MorphingFilterBar(
  ...
  useEffect(() => {
    const updatePosition = () => {
      ...
      const headerRight = buttonsRect
        ? window.innerWidth - buttonsRect.left + 16
        : 120;
  ...
  const skeletonContent = (
    <div
      className={`flex items-center ${isCompact || isIconOnly ? "gap-1.5" : "gap-2 sm:gap-4"}`}
    >
      {isLoggedIn && (
        <Skeleton
          className={isCompact || isIconOnly ? "h-8 w-8" : "h-9 w-[88px]"}
        />
      )}
      <Skeleton
        className={isCompact || isIconOnly ? "h-8 w-8" : "h-9 w-[110px]"}
      />
      <Skeleton
        className={isCompact || isIconOnly ? "h-8 w-8" : "h-9 w-[130px]"}
      />
      <Skeleton
        className={isCompact || isIconOnly ? "h-8 w-8" : "h-9 w-[90px]"}
      />
    </div>
  );
    ...
    >
      ...
      <Select value={sortBy} onValueChange={onSortChange}>
        <SelectTrigger
          ...
          className={`w-fit transition-all duration-100 ${isCompact || isIconOnly ? "text-xs" : ""}`}
        >
          <div className="flex items-center gap-2">
            <SortIcon
              className={`text-muted-foreground ${isCompact || isIconOnly ? "h-3.5 w-3.5" : "h-4 w-4"}`}
            />
      ...
      >
        <Filter
          className={isCompact || isIconOnly ? "h-3.5 w-3.5" : "h-4 w-4"}
        />
        {!isIconOnly && "Filters"}
        {activeFilterCount > 0 && (
          <Badge
            variant="secondary"
            className={isCompact || isIconOnly ? "text-[10px]" : "text-xs"}
          >
            {activeFilterCount}
          </Badge>
        )}
