...
describe("bounded durable reconciliation", () => {
  test("AutoRecycler quarantines missing geography across resume and preserves observed inventory through validation and reconciliation", async () => {
    ...
    try {
      ...
      for (const vin of [presentVin, "VIN-ABSENT"]) {
        await client.execute({
          ...
          args: [vin, org, firstSeenAt, firstSeenAt, firstSeenAt],
        });
      }
      ...
      for (const startFrom of [0, 3]) {
        ...
        expect(result.cursor).toBe(startFrom === 0 ? 3 : 106);
